...
python() {
    ...
    # execute the tasks only when LMP_BOOT_FIRMWARE_FILES is not clean
    if d.getVar('LMP_BOOT_FIRMWARE_FILES') == "":
        for tasks in ['do_install', 'do_deploy']:
            d.setVarFlag(tasks, 'noexec', '1')
}
